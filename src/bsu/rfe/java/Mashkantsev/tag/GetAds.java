package bsu.rfe.java.Mashkantsev.tag;
...
import bsu.rfe.java.Mashkantsev.entity.Ad;
import bsu.rfe.java.Mashkantsev.entity.AdList;
import bsu.rfe.java.Mashkantsev.entity.User;
...
// В этом списке будут содержаться только отобранныеобъявления
            ArrayList<Ad> sortedList = new ArrayList<Ad>();
// Проанализировать все объявления на доске объявлений
            for (Ad ad: adList.getAds()) {
// Если режим фильтрации собственные сообщенийвыключен
...
// Как анонимный класс определить и создать экземпляркомпаратора
// Именно компаратор будет отвечать за сортировку объявленийзаданным способом
            Comparator<Ad> comparator = new Comparator<Ad>() {
                public int compare(Ad ad1, Ad ad2) {
                    int result;
// Если выбрана сортировка по дате последнегоизменения объявления
                    if (GetAds.this.sort!=null &&
                            GetAds.this.sort.equals("date")) {
// То результат определяется по значениямполя lastModified
                        result =
...
// Иначе сортируем по автору объявления позначениям поля name автора
                            result =
